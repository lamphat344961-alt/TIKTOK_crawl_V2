...
class DBManager:
    ...
    def _table_columns(self, table_name: str) -> set[str]:
        ...
        try:
            ...
            cols = {row[0].upper() for row in self.cursor.fetchall()}
        except Exception as e:
    ...
    def upsert_creator(self, creator_id: str, source_doc: dict, profile_stats: dict):
        ...
        row = {
            ...
            "FOLLOWERS": profile_stats.get("followers_count") if profile_stats.get("followers_count") is not None else self._to_int(source_doc.get("Followers")),
    ...
    def upsert_video(self, creator_id: str, stats: dict):
        ...
        # SHARE_COUNT chỉ set nếu stats có field tương ứng
    ...
    def upsert_comments(
        ...
        try:
            for c in comments:
                cid = c.get("cid")
                comment_id = str(cid) if cid else make_comment_id(str(video_id), str(c.get("create_time") or ""), c.get("text") or "")
    ...
    def upsert_replies(
        ...
        try:
            for r in replies:
                cid = r.get("cid")
                reply_id = str(cid) if cid else make_reply_id(str(parent_comment_id), str(r.get("create_time") or ""), r.get("text") or "")
